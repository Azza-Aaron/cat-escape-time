...
export class PlayScene extends Phaser.Scene {
  ...
  private isPointerDown(): boolean {
    return this.useMobileTouch && this.input.activePointer.isDown;
  }

  private leftPressed(): boolean {
    return (
      (this.isPointerDown() && this.input.activePointer.worldX < this.player.x - 20) ||
  ...
  private upPressed(): boolean {
    return (
      (this.isPointerDown() && this.input.activePointer.worldY < this.player.y - 40) ||
  ...
  private consumeJumpPress(): boolean {
    ...
    let touchJump = false;
    if (this.isPointerDown() && this.input.activePointer.worldY < this.player.y - 40) {
      touchJump = true;
    }
    const touchJumpEdge = touchJump && !this.touchUpPrevJump;
    this.touchUpPrevJump = touchJump;

    return keyboardJump || touchJumpEdge;
  }
  ...
  private async gameOver(reason: string): Promise<void> {
    ...
    // Stop hud scene first so menus don't overlap
  ...
  update(time: number): void {
    ...
    const sprint = true; // Run by default as requested
